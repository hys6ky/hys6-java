...
# 加工程序Main方法
MAIN_CLASS="hyren.serv6.h.process_flink.run.ProcessMainJob"
# HADOOP_OPTS
#HADOOP_OPTS="-Djava.library.path=/opt/cloudera/parcels/CDH/lib/hadoop/lib/native"
# Get system bits
OS_BIT=$(getconf LONG_BIT)
# 脚本标签
JOB_TAG="PROCESS_FLINK_JOB"
# 模型表id_作业id
MODULE_TABLE_ID="${1}"
shift
# 模型表id_作业id
...
function usage() {
  echo
  echo "Usage       : bash $(basename $0) MODULE_TABLE_ID JOB_TABLE_ID JOB_NAME [Options]"
...
[ "Null$JOB_NAME" == "Null" ] && usage "ERROR: Missing parameter:3 <JOB_NAME>"
...
function collect_main() {
  ...
  echo ${PROCESS_JAR_NAME} ${MODULE_TABLE_ID} ${JOB_TABLE_ID} ${JOB_NAME} ${SQL_PARAMS}
  nohup "${SH_EXEC_DIR}"/../jre/linux/"${OS_BIT}"/jre/bin/java -Xms64m -Xmx1024m \
    ...
    ${MODULE_TABLE_ID} ${JOB_TABLE_ID} ${JOB_NAME} ${SQL_PARAMS} \
    >>"${SH_EXEC_DIR}"/job_"${JOB_TABLE_ID}" 2>&1 &
}
