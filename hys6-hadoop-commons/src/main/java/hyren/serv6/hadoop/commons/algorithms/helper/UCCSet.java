...
public class UCCSet {
    ...
    public UCCSet(int numAttributes, int maxDepth) {
        ...
        for (int i = 0; i <= numAttributes; i++) this.uccLevels.add(new ObjectOpenHashSet<OpenBitSet>());
    }
    ...
    public void trim(int newDepth) {
        while (this.uccLevels.size() > (newDepth + 1)) this.uccLevels.remove(this.uccLevels.size() - 1);
        this.depth = newDepth;
    ...
    public void clear() {
        ...
        for (int i = 0; i <= numLevels; i++) this.uccLevels.add(new ObjectOpenHashSet<OpenBitSet>());
    }

    public int size() {
        int size = 0;
        for (ObjectOpenHashSet<OpenBitSet> uccs : this.uccLevels) size += uccs.size();
        return size;
    }
}
