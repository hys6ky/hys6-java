...
public class UCCList {
    ...
    public UCCList(int numAttributes, int maxDepth) {
        ...
        for (int i = 0; i <= numAttributes; i++) this.uccLevels.add(new ArrayList<OpenBitSet>());
    }
    ...
    public void clear() {
        ...
        for (int i = 0; i <= numLevels; i++) this.uccLevels.add(new ArrayList<OpenBitSet>());
    }

    public int size() {
        int size = 0;
        for (List<OpenBitSet> uccs : this.uccLevels) size += uccs.size();
        return size;
    }
}
