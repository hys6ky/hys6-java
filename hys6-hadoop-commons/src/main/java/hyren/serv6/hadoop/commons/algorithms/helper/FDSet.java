...
public class FDSet implements Serializable {
    ...
    public FDSet(int numAttributes, int maxDepth) {
        ...
        for (int i = 0; i <= numAttributes; i++) this.fdLevels.add(new ObjectOpenHashSet<OpenBitSet>());
    }
    ...
    public void trim(int newDepth) {
        while (this.fdLevels.size() > (newDepth + 1)) this.fdLevels.remove(this.fdLevels.size() - 1);
        this.depth = newDepth;
    ...
    public void clear() {
        ...
        for (int i = 0; i <= numLevels; i++) this.fdLevels.add(new ObjectOpenHashSet<OpenBitSet>());
    }

    public int size() {
        int size = 0;
        for (ObjectOpenHashSet<OpenBitSet> uccs : this.fdLevels) size += uccs.size();
        return size;
    }
}
