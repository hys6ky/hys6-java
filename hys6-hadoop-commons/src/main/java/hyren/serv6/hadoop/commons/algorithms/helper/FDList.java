...
public class FDList {
    ...
    public FDList(int numAttributes, int maxDepth) {
        ...
        for (int i = 0; i <= numAttributes; i++) this.fdLevels.add(new ArrayList<OpenBitSet>());
    }
    ...
    public void clear() {
        ...
        for (int i = 0; i <= numLevels; i++) this.fdLevels.add(new ArrayList<OpenBitSet>());
    }

    public int size() {
        int size = 0;
        for (List<OpenBitSet> uccs : this.fdLevels) size += uccs.size();
        return size;
    }
}
