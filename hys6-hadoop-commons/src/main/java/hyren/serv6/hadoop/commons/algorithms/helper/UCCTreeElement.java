...
public class UCCTreeElement {
    ...
    public boolean hasChildren() {
        if (this.children == null)
            return false;
        for (int i = 0; i < this.children.length; i++) if (this.children[i] != null)
            return true;
        return false;
    }
    ...
    protected boolean containsUCCOrSpecialization(OpenBitSet ucc, int currentUCCAttr) {
        ...
        if (this.children == null)
            return false;
        if (currentUCCAttr < 0) {
            for (int childAttr = 0; childAttr < this.children.length; childAttr++) if ((this.children[childAttr] != null) && (this.children[childAttr].containsUCCOrSpecialization(ucc, currentUCCAttr)))
                return true;
        } else {
            int nextUCCAttr = ucc.nextSetBit(currentUCCAttr + 1);
            if (this.children[currentUCCAttr] != null)
    ...
    public int writeUniqueColumnCombinations(OutputStreamWriter receiver, OpenBitSet ucc, ObjectArrayList<ColumnIdentifier> columnIdentifiers, boolean writeTableNamePrefix) throws IOException {
        int numUCCs = 0;
        if (this.isUCC) {
        ...
        for (int childAttr = 0; childAttr < this.numAttributes; childAttr++) {
            ...
            if (element != null) {
                ucc.set(childAttr);
                numUCCs += element.writeUniqueColumnCombinations(receiver, ucc, columnIdentifiers, writeTableNamePrefix);
                ucc.clear(childAttr);
            }
        }
        return numUCCs;
