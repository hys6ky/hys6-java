...
@Slf4j
public class CsvBulkLoadJob extends Configured implements Tool {
    ...
    public static class BulkLoadMap extends Mapper<LongWritable, Text, ImmutableBytesWritable, Put> {
        ...
        public void map(LongWritable key, Text value, Context context) throws IOException, InterruptedException {
            ...
            for (int i = 0; i < headByte.size(); i++) {
                put.addColumn(Constant.HBASE_COLUMN_FAMILY, headByte.get(i), Bytes.toBytes(lines[i]));
            }
    ...
    public int run(String[] args) throws Exception {
        ...
        log.info("Arguments: " + todayTableName + "  " + hdfsFilePath + "  " + columnMetaInfo + "  " + rowKeyIndex + "  " + configPath + "  " + etlDate + "  " + isMd5 + "  " + hadoop_user_name + "  " + platform + "  " + prncipal_name + "  " + is_header);
        try (HBaseOperator hBaseOperator = new HBaseOperator(configPath, platform, prncipal_name, hadoop_user_name)) {
            ...
            Job job = Job.getInstance(hBaseOperator.conf, "CsvBulkLoadJob_" + todayTableName);
