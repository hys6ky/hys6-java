...
import hyren.serv6.f.source.CheckParam;
...
public class TableRegisterService {
    ...
    public void saveTableData(long source_id, long databaseId, TableDataReq[] tableDataReqs, String tableColumns, long dsl_id) {
        Object collect_type = checkDatabaseSetExist(databaseId);
        Map<String, List<Map<String, Object>>> tableColumnObj = null;
        if (StringUtil.isNotBlank(tableColumns) && !tableColumns.equals("{}")) {
            try {
        ...
        for (TableDataReq tableDataReq : tableDataReqs) {
            TableInfo tableInfo = new TableInfo();
            BeanUtil.copyProperties(tableDataReq, tableInfo);
            ...
            if (!collect_type.equals(CollectType.ShuJuKuCaiJi.getCode())) {
                saveStorageData(source_id, databaseId, dsl_id, tableInfo, tableDataReq.getIs_prefix());
            }
        }
        if (!collect_type.equals(CollectType.ShuJuKuCaiJi.getCode())) {
    ...
    @Param(name = "databaseId", desc = "", range = "")
    @Return(desc = "", range = "")
    public List<TableDataReq> getTableData(long databaseId) {
        checkDatabaseSetExist(databaseId);
        return Dbo.queryList(TableDataReq.class, "select ti.*,tsi.is_prefix " + "from " + TableInfo.TableName + " ti left join " + TableStorageInfo.TableName + " tsi " + "on ti.table_id = tsi.table_id where ti.database_id = ?", databaseId);
    }
    ...
    @Method(desc = "", logicStep = "")
    private void saveStorageData(long source_id, long databaseId, long dsl_id, TableInfo tableInfo, String isPrefix) {
        Map<String, Object> classifyAndSourceNum = getClassifyAndSourceNum(databaseId);
        String hyren_name = tableInfo.getTable_name().toLowerCase();
        if (IsFlag.Shi.getCode().equals(isPrefix)) {
            hyren_name = String.format("%s_%s_%s", classifyAndSourceNum.get("datasource_number"), classifyAndSourceNum.get("classify_num"), tableInfo.getTable_name()).toLowerCase();
        }
        long countNum = Dbo.queryNumber("SELECT COUNT(1) FROM " + DataStoreReg.TableName + " WHERE lower(hyren_name) = ? ", hyren_name).orElseThrow(() -> new BusinessException("SQL查询异常"));
        if (countNum != 0) {
            throw new BusinessException("当前表:" + tableInfo.getTable_name() + " 已存在");
        }
        ...
        table_storage_info.setIs_prefix(isPrefix);
    ...
    private List<TableColumn> databaseTableColumnInfo(long databaseId, String table_name) {
        ...
        Map<String, Object> databaseSetInfo = Dbo.queryOneObject(" select t1.dsl_id,  t1.fetch_size," + "  t1.agent_id, t1.db_agent, t1.plane_url" + " from " + DatabaseSet.TableName + " t1 " + " where t1.database_id = ? ", databaseId);
        DatabaseSet databaseSet = SendMsgUtil.getLegalParam(databaseSetInfo);
        try (DatabaseWrapper db = ConnectionTool.getDBWrapper(Dbo.db(), databaseSet.getDsl_id())) {
            List<TableMeta> tablesWithColumns = MetaOperator.getTablesWithColumns(db, table_name);
    ...
    @Param(name = "dsl_id", desc = "", range = "")
    @Param(name = "tableColumns", desc = "", range = "", nullable = true)
    public void updateTableData(long source_id, long databaseId, long dsl_id, TableDataReq[] tableDataReqs, String tableColumns) {
        Object collect_type = checkDatabaseSetExist(databaseId);
        if (!collect_type.equals(CollectType.ShuJuKuCaiJi.getCode())) {
    ...
    Map<String, Object> getClassifyAndSourceNum(long database_id) {
        return Dbo.queryOneObject("SELECT t2.classify_num,t3.datasource_number FROM " + DatabaseSet.TableName + " t1 JOIN " + CollectJobClassify.TableName + " t2 ON t1.classify_id = t2.classify_id JOIN " + DataSource.TableName + " t3 ON t2.source_id = t3.source_id WHERE t1.database_id = ?", database_id);
    }
}
