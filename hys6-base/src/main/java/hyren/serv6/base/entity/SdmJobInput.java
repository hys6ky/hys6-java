...
@Data
@ApiModel("StreamingPro作业输入信息表")
@Table(tableName = "sdm_job_input")
public class SdmJobInput extends ProEntity {
    ...
    @NotBlank(message = "")
    protected String input_en_name;

    @ApiModelProperty(value = "", required = false)
    protected String input_cn_name;

    @ApiModelProperty(value = "", required = true)
    @Size(min = 1, max = 64, message = "")
    @NotBlank(message = "")
    protected String input_table_name;

    @ApiModelProperty(value = "", required = true)
    @Size(min = 1, max = 512, message = "")
    @NotBlank(message = "")
    protected String input_source;

    @ApiModelProperty(value = "", required = true)
    @Size(min = 1, max = 1, message = "")
    @NotBlank(message = "")
    protected String input_data_type;
    ...
    public void setInput_number(String input_number) {
        if (fd.ng.core.utils.StringUtil.isNotBlank(input_number)) {
