package hyren.serv6.base.entity;

import io.swagger.annotations.ApiModel;
import javax.validation.constraints.NotBlank;
import javax.validation.constraints.Size;
import fd.ng.db.entity.anno.Table;
import io.swagger.annotations.ApiModelProperty;
import hyren.serv6.base.entity.fdentity.ProEntity;
import lombok.Data;
import java.math.BigDecimal;
import java.util.Set;
import java.util.HashSet;
import java.util.Collections;

@Data
@ApiModel("接口使用信息表")
@Table(tableName = "interface_use")
public class InterfaceUse extends ProEntity {

    private static final long serialVersionUID = 321566870187324L;

    private transient static final Set<String> __PrimaryKeys;

    public static final String TableName = "interface_use";

    public static boolean isPrimaryKey(String name) {
        return __PrimaryKeys.contains(name);
    }

    public static Set<String> getPrimaryKeyNames() {
        return __PrimaryKeys;
    }

    static {
        Set<String> __tmpPKS = new HashSet<>();
        __tmpPKS.add("interface_use_id");
        __PrimaryKeys = Collections.unmodifiableSet(__tmpPKS);
    }

    @ApiModelProperty(value = "", required = true)
    protected Long interface_use_id;

    @ApiModelProperty(value = "", required = true)
    @Size(min = 1, max = 512, message = "")
    @NotBlank(message = "")
    protected String url;

    @ApiModelProperty(value = "", required = true)
    @Size(min = 1, max = 512, message = "")
    @NotBlank(message = "")
    protected String interface_name;

    @ApiModelProperty(value = "", required = true)
    @Size(min = 1, max = 100, message = "")
    @NotBlank(message = "")
    protected String interface_code;

    @ApiModelProperty(value = "", required = true)
    @Size(min = 1, max = 1, message = "")
    @NotBlank(message = "")
    protected String their_type;

    @ApiModelProperty(value = "", required = true)
    @Size(min = 1, max = 512, message = "")
    @NotBlank(message = "")
    protected String user_name;

    @ApiModelProperty(value = "", required = false)
    protected String classify_name;

    @ApiModelProperty(value = "", required = true)
    protected Long create_id;

    @ApiModelProperty(value = "", required = true)
    @Size(min = 1, max = 1, message = "")
    @NotBlank(message = "")
    protected String use_state;

    @ApiModelProperty(value = "", required = true)
    @Size(min = 1, max = 8, message = "")
    @NotBlank(message = "")
    protected String start_use_date;

    @ApiModelProperty(value = "", required = true)
    @Size(min = 1, max = 8, message = "")
    @NotBlank(message = "")
    protected String use_valid_date;

    @ApiModelProperty(value = "", required = false)
    protected String interface_note;

    @ApiModelProperty(value = "", required = true)
    protected Long interface_id;

    @ApiModelProperty(value = "", required = true)
    protected Long user_id;

    public void setInterface_use_id(String interface_use_id) {
        if (fd.ng.core.utils.StringUtil.isNotBlank(interface_use_id)) {
            this.interface_use_id = new Long(interface_use_id);
        }
    }

    public void setInterface_use_id(Long interface_use_id) {
        this.interface_use_id = interface_use_id;
    }

    public void setCreate_id(String create_id) {
        if (fd.ng.core.utils.StringUtil.isNotBlank(create_id)) {
            this.create_id = new Long(create_id);
        }
    }

    public void setCreate_id(Long create_id) {
        this.create_id = create_id;
    }

    public void setInterface_id(String interface_id) {
        if (fd.ng.core.utils.StringUtil.isNotBlank(interface_id)) {
            this.interface_id = new Long(interface_id);
        }
    }

    public void setInterface_id(Long interface_id) {
        this.interface_id = interface_id;
    }

    public void setUser_id(String user_id) {
        if (fd.ng.core.utils.StringUtil.isNotBlank(user_id)) {
            this.user_id = new Long(user_id);
        }
    }

    public void setUser_id(Long user_id) {
        this.user_id = user_id;
    }
}
