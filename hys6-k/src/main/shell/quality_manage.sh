...
# 采集程序Main方法
MAIN_CLASS="hyren.serv6.k.biz.RuleCMDExecute"
# HADOOP_OPTS
...
function collect_main() {
  ...
  CLASSPATH="${CLASSPATH}${SH_EXEC_DIR}/../jdbc/*"
    ...
    -cp "${CLASSPATH}" -Dloader.main="${MAIN_CLASS}"  org.springframework.boot.loader.PropertiesLauncher "${TASK_ID}" "${ETL_DATE}"
...
function usage() {
  ...
  echo 'for example: quality_manage.sh "899948182551662592" "20230303"'
  exit
}
