...
echo ${SH_EXEC_DIR}
# 采集包名
COLLECT_JAR_NAME="hyren-serv6-k-${RELEASE_VERSION}.jar"
# 采集程序Main方法
MAIN_CLASS="hyren.serv6.k.standard.standardTask.batch.StandardBatchMain"
# HADOOP_OPTS
...
#    -Dproject.id="${ETL_DATE}" \
#    -DisMultiLineLog=true \
#    -Djava.ext.dirs="${SH_EXEC_DIR}"/../jdbc:"${SH_EXEC_DIR}"/../jre/linux/"${OS_BIT}"/jre/lib/ext \
#    -cp "${CLASSPATH}" -Dloader.main="${MAIN_CLASS}"  org.springframework.boot.loader.PropertiesLauncher "${ETL_DATE}"
...
function usage() {
  echo "Usage: $0 ETL_DATE"
  echo 'for example: standard_job.sh "20230101"'
  exit
}
