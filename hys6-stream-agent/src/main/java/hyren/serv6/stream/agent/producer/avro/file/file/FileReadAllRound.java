...
public class FileReadAllRound implements Runnable {
    ...
    public FileReadAllRound(String jobId, Map<String, Object> json, CountDownLatch countDownLatch) {
        ...
        Runtime.getRuntime().addShutdownHook(new Thread(() -> {
            quit();
            logger.error("FileReadAllRound------------------FileReadAllRound shutdown hook down." + System.currentTimeMillis());
        }));
    }
    ...
    public void startup() {
        logger.info("FileReadAllRound---------------------FileReadAllRound startup.");
        ...
        try {
            ...
            Thread thread = FileContentStreamInfoController.mapJob.get(jobId);
            if (thread != null) {
                thread.interrupt();
            }
            runnerFuture = executor.submit(runner);
        } catch (Exception e) {
            logger.error("FileReadAllRound-------------startup failed!", e);
            quit();
        }
    }
    ...
    public void quit() {
        ...
        logger.info("FileReadAllRound-----------------FileReadAllRound quit step 1 runner killed.");
        if (runnerFuture != null) {
            runnerFuture.cancel(true);
        }
        logger.info("FileReadAllRound-------------------FileReadAllRound quit step 2 runnerFuture canceled.");
        executor.shutdown();
        logger.info("FileReadAllRound-----------------------FileReadAllRound quit step 3 executor shutdown.");
    ...
    private class ArtisanRunnable implements Runnable {
        ...
        public ArtisanRunnable(String is_data_partition, boolean readeOpinion, String filepath, long readSeekPos, String charset, FileDataValidator filedataValidator, String file_read_num, JobParamsEntity jobParams, CountDownLatch countDownLatch, String read_mode, String is_obj) {
        ...
        public void run() {
            ...
            if (!this.file.exists()) {
                logger.error("FileReadAllRound------------------file [{}] is not exist! " + filepath);
                return;
            }
            ...
            if (this.readSeekPos == READ_POSITION_TAIL) {
                ...
                try (RandomAccessFile rafFilePointer = new RandomAccessFile(new File(filepath + ".rdp"), "r")) {
                    ...
                    if (e instanceof FileNotFoundException)
                        ...
                        logger.error("FileReadAllRound-------------read rdp file [" + this.filepath + "] failed!", e);
                        return;
                    }
                }
                ...
                logger.error("FileReadAllRound--------------------file read position[{}] is wrong! " + this.readSeekPos);
                return;
            }
                        ...
                        if (isNewLine(curLineString, filedataValidator)) {
                            if (lineBuffer.length() < 1) {
                                ...
                                if (kafkaProducerWorkerAvro.sendToKafka(filepath, producer, genericRecord, topic, cp, bootstrapServers, sync)) {
                                    ...
                                    logger.error("FileReadAllRound-----------数据发送失败！！！");
                                    break;
                                }
                            }
                        } else {
