...
public class ProducerOperatorAvro {
    ...
    public JobParamsEntity getMapParam(Map<String, Object> json) {
        ...
        try {
            ...
            if (null != msgtypeJson.get("header")) {
                header = String.valueOf(msgtypeJson.get("header"));
            }
            if (header != null) {
            ...
            for (int key : mapSchema.keySet()) {
                schemaString = schemaString + mapSchema.get(key);
            }
            ...
            if (SdmPatitionWay.Key == SdmPatitionWay.ofEnumByCode(sdm_partition)) {
                CusClassLoader classLoader = new CusClassLoader();
                CustomerPartition cp = (CustomerPartition) classLoader.getURLClassLoader().loadClass(sdm_partition_name).newInstance();
            ...
            logger.info("ProducerOperatorAvro----------------------KafkaProducerWorker加载配置文件！！！");
        } catch (Exception e) {
            if (e instanceof BusinessException) {
                throw new BusinessException(e.getMessage());
            }
            logger.error("ProducerOperatorAvro-------------------生产者参数获取失败！！！", e);
        }
        if (properties != null) {
