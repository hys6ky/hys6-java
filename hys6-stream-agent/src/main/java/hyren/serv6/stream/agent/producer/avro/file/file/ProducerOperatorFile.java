...
public class ProducerOperatorFile {
    ...
    public JobParamsEntity getMapParam(String is_data_partition, Map<String, Object> json, String jobId) {
        ...
        try {
            ...
            List<Map<String, Object>> columnsJson = (List<Map<String, Object>>) json.get("messInfoList");
            Map<String, Object> params = (Map<String, Object>) json.get("sdm_receive_conf");
            String is_obj = null;
            if (null != params.get("is_obj")) {
                is_obj = params.get("is_obj").toString();
            }
            String read_mode = null;
            if (null != params.get("read_mode")) {
                read_mode = params.get("read_mode").toString();
                ;
            }
            ...
            for (Object columnParam : columnsJson) {
                ...
                if (null != columnJson.get("sdm_var_type")) {
                    type = columnJson.get("sdm_var_type").toString();
                }
            ...
            if ("1".equals(read_mode)) {
                if (columnsJson.size() > 0 && IsFlag.Shi == IsFlag.ofEnumByCode(is_data_partition)) {
                ...
                if (columnsJson.size() > 0 && IsFlag.Shi == IsFlag.ofEnumByCode(is_obj)) {
                    for (int key : mapSchema.keySet()) {
                        schemaString.append(mapSchema.get(key));
                    }
                    schemaMessage = schemaString.substring(0, schemaString.length() - 1) + "]}";
            ...
            logger.info("ProducerOperatorFile--------------KafkaProducerWorker加载配置文件！！！");
        } catch (Exception e) {
            if (e instanceof BusinessException) {
                throw new BusinessException(e.getMessage());
            }
            logger.error("ProducerOperatorFile-----------------生产者参数获取失败！！！", e);
        }
        if (properties != null) {
            try {
