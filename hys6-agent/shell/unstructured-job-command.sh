...
# 采集程序Main方法
MAIN_CLASS="hyren.serv6.agent.run.UnstructuredJobCommand"
# HADOOP_OPTS
HADOOP_OPTS="-Djava.library.path=/opt/cloudera/parcels/CDH/lib/hadoop/lib/native"
# Get system bits
OS_BIT=$(getconf LONG_BIT)
# 脚本标签
COLLECT_TAG="HYREN_UNSTRUCTURED_JOB"
# 采集任务id
TASK_ID="${1}"

# 脚本运行入口
## 参数1：任务ID
## 使用方式 sh unstructured-collection.sh 899948182551662592
## 返回状态码说明 {1: 程序包不存在}
main() {
...
function usage() {
  ...
  echo "for example: $0 899948182551662592"
  exit
}
