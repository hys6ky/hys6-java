...
public class StorageLayerUtil {
    ...
    public static Map<String, Object> getStorageLayerConfInfo(DatabaseWrapper db, long dsl_id) {
        ...
        DataStoreLayer dsl = Dbo.queryOneObject(db, DataStoreLayer.class, "select * from " + DataStoreLayer.TableName + " where dsl_id=?", dsl_id).orElseThrow(() -> new SystemBusinessException("获取存储层信息失败! dsl_id=" + dsl_id));
        slci.put("dsl", dsl);
        ...
        if (Store_type.HIVE == Store_type.ofEnumByCode(store_type) || Store_type.DATABASE == Store_type.ofEnumByCode(store_type)) {
            ...
            if (databaseTypes.isEmpty()) {
                throw new SystemBusinessException("该存储类型或者数据库没有配置类型列表数据，请联系管理员处理！！！" + database_name);
            }
            ...
            throw new SystemBusinessException("该存储类型没有配置类型列表数据，请联系管理员处理！！！" + Store_type.ofEnumByCode(store_type));
        }
        return slci;
    }
}
